...
Such a setup covers all four CRUD operations, making the case study representative of real-life applications.
...
Each requirement RX.Y is assigned a score $R_{\text{X.Y}}$ based on the formula~\ref{eq:3-4-requirement-scoring}:
\begin{equation}
    R_{\text{X.Y}} =
    \begin{dcases}
...
The user can navigate to each board by clicking a card with its name.
...
The top and bottom parts are \enquote{stuck} to the edges of the container: the title is always on the top of the container;
