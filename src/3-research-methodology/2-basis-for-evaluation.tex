...
\subsubsection{View}
An application consists of \emph{views}/\emph{screens} -- collections of controls that are used together in order to fulfill a certain use case.
...
This classification describes if the element can contain and handle children components -- in that case, it is called a \emph{container}.
...
The classification does not have to be binary -- for example, a tabbed layout\furl{https://developer.apple.com/documentation/appkit/nstabview\#2555818} contains other content, but also has its own controls responsible for switching tabs.
...
An additional aspect that needs to be considered when defining custom components is the mechanism of input parameters -- they are necessary to make components manageable, reusable and useful in different contexts.
...
Additionally, it's a good practice nowadays to make interface layouts accommodate displays of different sizes and fit accordingly -- it is called \emph{responsive design}.
...
Finally, some descriptions make it possible to specify the appearance of portions of applications in probably most comprehensive and robust way -- by using CSS directly.
