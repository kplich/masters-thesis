...
This section elaborates on the classification from the previous section and defines the criteria for the evaluation of UI descriptions.
...
Table~\ref{tab:evaluation-criteria-behavior} presents important areas of modeling GUI behavior, complemented by the list of components that representations might support out-of-the-box, presented in Table~\ref{tab:evaluation-criteria-components}.
...
Second-level criteria are indicated by adding an extra number after the top-level criterion symbol and are written using a regular font (B1.3);
...
Table ~\ref{tab:my-table} presents example results of an evaluation of two hypothetical representations (R1 and R2) against a set of criteria.
...
Criterion \textbf{A1} does not contain any subcriteria, therefore it is evaluated directly.
Criterion \textbf{A2} contains two subcriteria, so the score for this criterion is a percentage of subcriteria that fulfill the given criteria -- in R1, only one of two criteria is satisfied, so the score is 50\%; in R2 both criteria are satisfied, so the score is 100\%.
The same mechanism applies to criteria A3.1, A3.2, and criterion \textbf{A3} -- its value is calculated as an average of scores for criteria A3.1 and A3.2.
...
usually, they're emitted by most elements.
...
One of the representations~\cite{Gaouar2018} also included some \enquote{system events} related to the functioning of a mobile device, e.g.\ indicating an incoming or outgoing call or a change in the state of a network connection.
...
Such an approach blends the two concepts, making them more difficult to separate from one another.
...
Criterion \textbf{C6} describes interaction components.
...
Criteria \textbf{D3}--\textbf{D4} list the expected methods of sizing and positioning of the elements.
...
Finally, criterion \textbf{D8} verifies if the representations allow for using CSS directly.
