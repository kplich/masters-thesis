...
This section elaborates on the previously described classification and defines the criteria for the evaluation of UI descriptions.
...
second-level criteria are indicated by adding a number after the top-level criterion symbol, are written using a regular font (B1.3), and are separated visually in the table using a dashed line.
...
Each lowest-level criterion (that does not contain subcriteria) in each area was assigned a score of 1 (if the representation fulfilled the criterion) or 0 (otherwise).
...
Table ~\ref{tab:my-table} presents example results for two hypothetical representations (R1 and R2): fulfilled criteria are marked with a checkmark (\cmark), the rest are marked with a cross mark (\xmark);
...
Criterion \textbf{A2} comprises two subcriteria, so the score for this criterion is a percentage of subcriteria that fulfill the given criteria -- in R1, only one of the two criteria is satisfied, so the score is 50\%; in R2 both criteria are satisfied, so the score is 100\%.
...
usually, they are emitted by most elements.
...
Criteria \textbf{D1} and \textbf{D2} evaluate the capabilities of describing layouts, including responsive ones.
Criteria \textbf{D3}--\textbf{D4} list the expected methods element of sizing and positioning.
